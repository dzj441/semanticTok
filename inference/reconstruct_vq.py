...
def main(args):
    ...
    if args.vq_model.endswith(".pt"):
        ckpt = torch.load(args.vq_model)
        VQBACKBONE = "SOFTVQVAE/softvq"
        vq_model = SoftVQModel.from_pretrained(VQBACKBONE)
        vq_model.load_state_dict(load_model_state_dict(ckpt['model']),strict=False)
        del ckpt
        print("load from .pt!")
    else:
    ...
    psnr_val_rgb = []
    ssim_val_rgb = []
    PATH = "../weights/weights-inception-2015-12-05-6726825d.pth"
    compute_fid_score = FrechetInceptionDistance(normalize=False,feature_extractor_weights_path=PATH).cuda()
    all_z = []
    all_zq = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vq-model", type=str, default="experiments/tokenizer/exp000-softvq-l-64/checkpoints/0250000.pt")
