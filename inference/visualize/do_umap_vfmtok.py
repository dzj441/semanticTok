...
def build_model(args, device):
    ...
    # ========= 全局硬编码配置 =========
    VQ_MODEL = "VQ-16"  # 对应 VQ_models 里的 key
    VQ_CKPT = "./weights/vfmtok-tokenizer.pt"  # 你的 tokenizer 权重路径

    CODEBOOK_SIZE = 16384
    CODEBOOK_SLOTS_EMBED_DIM = 12
    Z_CHANNELS = 512

    TRANSFORMER_CONFIG_FILE = "configs/vfmtok_transformer.yaml"

    # "cuda" 或 "cpu"
    DEVICE = "cuda"   # 如果你想强制在 CPU 上就写 "cpu"


    transformer_config = OmegaConf.load(TRANSFORMER_CONFIG_FILE)

    # 2) 构建模型
    model = VQ_models[VQ_MODEL](
        codebook_size=CODEBOOK_SIZE,
        z_channels=Z_CHANNELS,
        codebook_slots_embed_dim=CODEBOOK_SLOTS_EMBED_DIM,
    ...
    drop_keys = [k for k in model_weight if "codebook_used" in k]
    for k in drop_keys:
        print(f"[Stage1] Removing buffer from ckpt: {k}")
        model_weight.pop(k)
