...
def run_eval(step, model_for_eval, ptdtype,  
    ...
    for images, _ in eval_loader:
        ...
        real_01 = torch.clamp((images + 1) * 0.5, 0, 1)
        fake_01 = torch.clamp((recons_imgs + 1) * 0.5, 0, 1)
...
def main(args):
    ...
    if rank == 0:
        ...
        eval_transform = transforms.Compose([
            transforms.Lambda(lambda pil_image: center_crop_arr(pil_image, args.image_size)),  # 固定中心裁剪，无随机增广
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5], inplace=True)
        ])
...
def build_parser():
    ...
    parser.add_argument("--transformer-config", type=str, default="configs/vfmtok_transformer.yaml",
                        help="OmegaConf path or dict for VFMTok encoder/decoder transformers")
