    ...
    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx, global_step, last_layer=None, 
        ...
        if optimizer_idx == 0:
            ...
            if global_step % log_every == 0:
                ...
                repa_loss_val = repa_loss if repa_loss is not None else 0.0
                cls_mse_val = cls_mse_loss if cls_mse_loss is not None else 0.0
                cls_cos_val = cls_cos_loss if cls_cos_loss is not None else 0.0

                logger_string = f"(Generator) rec_loss: {rec_loss:.4f}, perceptual_loss: {p_loss:.4f}, " \
                                f"codebook_loss: {codebook_loss[0]:.4f}, commit_loss: {codebook_loss[1]:.4f}, entropy_loss: {codebook_loss[2]:.4f}, repa_loss: {repa_loss_val:.4f}"
                if cls_mse_loss is not None:
                    logger_string += f", cls_l2: {cls_mse_val:.4f}"
                if cls_cos_loss is not None:
                    logger_string += f", cls_cos: {cls_cos_val:.4f}"
                ...
                if extra_losses:
                    for idx, item in enumerate(extra_losses, start=7):
                ...
                if tdist.get_rank() == 0 and self.wandb_logger is not None:
                    ...
                    if cls_mse_loss is not None:
                        log_dict['cls_loss_l2'] = cls_mse_val
                    if cls_cos_loss is not None:
                        log_dict['cls_loss_cos'] = cls_cos_val
                    if extra_losses:
                        for idx, item in enumerate(extra_losses, start=7):
