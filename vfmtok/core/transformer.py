...
from .deformable_transformer import DeformableTransformerEncoderLayer, CrossAttnEncoderLayer
...
class VanillaCrossAttnEncoder(nn.Module):
    """Encoder that replaces deformable attention with vanilla cross-attention over flattened multi-scale features."""

    def __init__(self, dim, depth, dim_feedforward=1024, dropout=0.,
                 activation="relu", n_heads=8):
        super().__init__()
        encoder_layer = CrossAttnEncoderLayer(
            d_model=dim,
            d_ffn=min(4 * dim, dim_feedforward),
            dropout=dropout,
    ...
    def forward(self, x, pos, kv_features, *unused_args):
        """Forward identical interface to deformable encoder for easy swap."""
        kv_tokens = []
        for feat in kv_features:
            b, c, h, w = feat.shape
            kv_tokens.append(feat.flatten(2).transpose(1, 2))
        kv_concat = torch.cat(kv_tokens, dim=1) if kv_tokens else None

        output = x.flatten(2).transpose(1, 2)
        for layer in self.layers:
            output = layer(output, pos, kv_concat)
        return output


class SelfAttnEncoder(nn.Module):
    """Encoder that concatenates multiscale features with learnable queries for pure self-attention."""

    def __init__(self, dim, depth, dim_head, mlp_dim, num_head=8, dropout=0.0):
        super().__init__()
        self.transformer = Transformer('normal', dim, depth, num_head, dim_head, mlp_dim, dropout, xformer=False)

    def forward(self, x, pos, kv_features):
        queries = x.flatten(2).transpose(1, 2)
        feat_tokens = [feat.flatten(2).transpose(1, 2) for feat in kv_features]
        concat = torch.cat(feat_tokens + [queries], dim=1)
        if pos is not None:
            concat[:, -queries.size(1):] += pos

        output = self.transformer(concat)
        return output[:, -queries.size(1):]
