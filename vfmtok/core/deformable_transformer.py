...
from ..modules.attention import Attention3,CrossAttention2
...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, pos, reference_points, kv_feature, input_spatial_shape, level_start_index):
        ...
        mask = ~torch.tril(src.new_ones((n_c, n_c), dtype=torch.bool), diagonal=0) # causal self attn
        with torch.autocast('cuda', dtype=torch.float32, enabled=True):
...
class CrossAttnEncoderLayer(nn.Module):
    """Encoder layer that swaps deformable attention for vanilla cross-attention."""
    ...
    def forward(self, src, pos, kv_feature):
        bs, n_c, dim = src.shape
        with torch.autocast('cuda', dtype=torch.float32, enabled=True):
            src2 = self.cross_attn(self.with_pos_embed(src, pos), context=kv_feature)
        src = src + self.dropout1(src2)
        src = self.norm1(src)
