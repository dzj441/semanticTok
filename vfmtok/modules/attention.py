...
class CrossAttention2(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        q = self.to_q(x)        # [B, N_q, H*D]
        k = self.to_k(context)  # [B, N_k, H*D]
        v = self.to_v(context)  # [B, N_k, H*D]

        q = rearrange(q, 'b n (h d) -> b h n d', h=h)      # [B, H, N_q, D]
        k = rearrange(k, 'b n (h d) -> b h n d', h=h)      # [B, H, N_k, D]
        v = rearrange(v, 'b n (h d) -> b h n d', h=h)      # [B, H, N_k, D]
        ...
        out = rearrange(out, 'b h n d -> b n (h d)')       # [B, N_q, H*D]
